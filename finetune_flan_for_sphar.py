...
from utils import (
    ...
    create_prompt,
    extend_prompts,
    custom_create_question,
)
from transformers.optimization import Adafactor, AdafactorSchedule

from data import VisualReasoningDatasetCustom, custom_collate, load_dataset
...
def train(
    ...
    for idx, data in enumerate(loader, 0):
        ...
        current_questions = custom_create_question(
            data,
            include_choices=args.include_choices,
            include_profile=args.include_profile,
            include_caption=args.include_caption,
        ...
        if args.include_rationale:
            ...
            if args.dataset_name in ["okvqa", "vqav2", "gqa", "clevr", "sphar", "sphar_custom"]:
                targets = [item["direct_answers"][0] for item in data]
            else:
                targets = extract_group(data, "mc_answer")
        # print(data, prompts, targets)
...
def main():
    ...
    wandb.init(config=args, project="fine_tune_flan", entity="white_eagle", name="sphar_custom_final")

    # tokenizer for encoding the text
    tokenizer = T5Tokenizer.from_pretrained("google/flan-t5-base")

    # Creating the Training and Validation dataset
    train_set = VisualReasoningDatasetCustom(
    ...
    val_set = VisualReasoningDatasetCustom(
        dataset_dir=args.data_dir,
        dataset_name=args.dataset_name,
