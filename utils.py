...
def parse_args():
    ...
    parser.add_argument(
        "--dataset-name",
        type=str,
        choices=["aokvqa", "vcr", "esnlive", "vsr", "okvqa", "vqav2", "sphar", "sphar_custom", "gqa", "clevr"],
        required=True,
    )
    parser.add_argument(
...
def custom_prompt_element(
    ...
    **kwargs,
    ...
    if include_question:
        prompt += (
            "Provide responses based on the descriptions provided by OFA and BLIP for each frame, even if the frames are not presented in the original video's chronological sequence."
            + "OFA and BLIP are two different vision-language models to provide clues. \n"
            + "\n"
        )
    prompt += (
        (
            f"OFA's description per frame: {kwargs['vlm2_captions'][d['question_id']]}\n"
            if include_caption
            else ""
        )
        + (
            f"BLIP's description per frame: {kwargs['vlm1_captions'][d['question_id']]}\n"
            if include_caption
            else ""
        )
        + f"Q: What action is being performed in the video from which frames have been extracted?"
        + "\n"
        + "Answer directly."
        + (f"A:")
    )
