...
def load_dataset(dataset_dir, split, dataset_name="aokvqa", version="v1p0"):
    ...
    elif dataset_name in ['sphar', 'sphar_custom']:
         dataset_path = os.path.join(
            dataset_dir, dataset_name, f"{dataset_name}_{split}.json"
        )
    else:
        dataset_path = os.path.join(
...
class VisualReasoningDataset(Dataset):
    def __init__(
        ...
        elif dataset_name == "sphar":
            self.image_path_finder = os.path.join(dataset_dir, "sphar")
    ...
    def preprocess_data(self, d):
        ...
        elif self.dataset_name in ["sphar"]:
            d["image_path"] = os.path.join(
                self.image_path_finder, d["extracted_frame_location"], f"{d['image_id']}.jpeg"
            )
        else:
...
class VisualReasoningDatasetCustom(Dataset):
    def __init__(
        self,
        dataset_dir: pathlib.Path,
        dataset_name: str,
